...
FROM(SELECT DISTINCT TITLE,URL,POPULARITATE FROM ARTICLES  ORDER BY POPULARITATE DESC) QUERIES WHERE ROWNUM <= 3
ORDER BY ROWNUM");
				ociexecute($query);
	
	$totaltopstiri="<li><button onclick=topStiri()>Top Stiri </button><button onclick=topStiri24()>Top 24h </button><button onclick=worst24()>Worst 24h </button></li><li><h4>top 3 stiri<h4></li>";
