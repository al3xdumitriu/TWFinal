...
$query = ociparse($conn,"SELECT * FROM EVIDENTAVOTURI WHERE IDART=$dbid AND USERNAME='$user'");
 ...
 $query = ociparse($conn,"SELECT * FROM ARTICLES WHERE IDART=$dbid ");
	...
	if($dbvot==4){
		$query = ociparse($conn,"INSERT INTO EVIDENTAVOTURI  VALUES('$user',$dbid,2)");
				ociexecute($query);
	...
	echo "$dbpop";
		...
		$query = ociparse($conn,"UPDATE EVIDENTAVOTURI SET VOTAT=2 WHERE IDART=$dbid AND USERNAME='$user' ");
				ociexecute($query);
		 $query = ociparse($conn,"UPDATE ARTICLES SET POPULARITATE=POPULARITATE+1 WHERE URL='$dburl' ");
				ociexecute($query);
	$dbpop=$dbpop+1;
